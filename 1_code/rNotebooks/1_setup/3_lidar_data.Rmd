...
{ 
  ...
  writeRaster(x, filename=paste0('0_data/manual/spatialCov/LiDAR_mosaics/',bl[j]), format="GTiff",  overwrite=T) 
...
lidar_list<-list.files(path='0_data/manual/spatialCov/LiDAR_mosaics/', pattern = "*.tif$", full.names = TRUE)
lidar_stack<-raster::stack(lidar_list)

#save stack as r object
stackSave(lidar_stack,file="0_data/manual/spatialCov/lidar_stack.stk")
...
#load stack
lidar_stack<-stackOpen("0_data/manual/spatialCov/lidar_stack.stk")
...
write.csv(ss_lidar_df, file = "0_data/manual/spatialCov/ss_lidar_df.csv")
```
...
```{r}
#list the files
lidar_list<-list.files(path='../../chapter_3/empirical/0_data/manual/mosaics/', pattern = "*.tif$", full.names = TRUE)
lidar_stack<-raster::stack(lidar_list)

#load("../..//chapter_3/empirical/0_data/manual/rObjects/lidar_stack.Rdata")
rasterOptions(progress = 'text', timer = TRUE)
ss_lidar_2<-extract(lidar_stack,ss_xy[1,],buffer=100, na.rm=TRUE, fun=mean) 
