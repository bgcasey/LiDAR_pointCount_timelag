...
bird_callingLake_long_sub$ABUND<-1
bird_callingLake_long_sub<-distinct(bird_callingLake_long_sub)

bird_callingLake_wide<-bird_callingLake_long_sub%>%
...
save(bird_callingLake_wide, file="0_data/manual/bird/bird_callingLake_wide.rData")
```
...
x_xy<-st_transform(ss_xy, crs=3400)
...
bird_callingLake_wide_5<-bird_callingLake_wide_4[c(1:14, 48, 97, 18)]
...
callingLake_QPAD_offsets <- tibble::rownames_to_column(qpad_offsets, "PKEY")%>%
      dplyr::select(PKEY, BTNW, MOWA, SWTH)%>%
      dplyr::rename(c(BTNW_OFF=BTNW, MOWA_OFF=MOWA, SWTH_OFF=SWTH))%>%
...
knitr::kable(head("0_data/manual/bird/callingLake_QPAD_offsets.rData")) %>%
  kable_styling(font_size = 10, bootstrap_options= c("striped", "hover", "condensed" ), full_width= F, position="center") %>%
  column_spec(1, width= "20em")%>%
  scroll_box(width = "100%", height = "400px")
```
