...
```{r eval=FALSE}
WIWR_m0<-glm(WIWR_OCC ~ 1, data=d, subset=SS_lidar_timelag==0, family = binomial, offset = WIWR_OFF,  na.action = na.exclude)

WIWR_m1<-lme4::glmer(WIWR_OCC ~ 1 + (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = WIWR_OFF,  na.action = na.exclude)

WIWR_m2<-lme4::glmer(WIWR_OCC ~ 1 + (1|SS) + (1|ss_year), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = WIWR_OFF,  na.action = na.exclude)

WIWR_m3<-lme4::glmer(WIWR_OCC ~ 1 + (1|ss_year/SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = WIWR_OFF,  na.action = na.exclude)
...
plm2<-lme4::glmer(WIWR_OCC ~ rast_forest_age + canopy_relief_ratio+ elev_0pnt15_to_2pnt00_return_proportion+ elev_2pnt00_to_4pnt00_return_proportion+ elev_cv+ elev_kurtosis+ elev_maximum + elev_mean+ elev_p05 + elev_p10 + elev_p20 + elev_p25 + elev_p30 + elev_p50 + elev_p60 + elev_p70 + elev_p75 + elev_p80 + elev_p95 + elev_p99 + elev_stddev+ percentage_first_returns_above_2pnt00+ percentage_first_returns_above_mean+ total_all_returns +  (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = WIWR_OFF,  na.action = na.exclude ) 

l_vif<-as.data.frame(vif(plm2))


plm3<-lme4::glmer(WIWR_OCC ~ rast_forest_age + canopy_relief_ratio+ elev_0pnt15_to_2pnt00_return_proportion+ elev_2pnt00_to_4pnt00_return_proportion+ elev_cv+ elev_kurtosis+ elev_maximum + elev_mean+ elev_p50 + elev_p95 + elev_p99 + elev_stddev+ percentage_first_returns_above_2pnt00+ percentage_first_returns_above_mean+ total_all_returns +  (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = WIWR_OFF,  na.action = na.exclude ) 

l_vif<-as.data.frame(vif(plm3))


plm4<-lme4::glmer(WIWR_OCC ~ canopy_relief_ratio+ elev_0pnt15_to_2pnt00_return_proportion+ elev_2pnt00_to_4pnt00_return_proportion+ elev_kurtosis+ elev_maximum +  elev_p50 + elev_p95 + elev_stddev+ percentage_first_returns_above_2pnt00+ percentage_first_returns_above_mean+ total_all_returns +  (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = WIWR_OFF,  na.action = na.exclude ) 

l_vif<-as.data.frame(vif(plm4))


plm5<-lme4::glmer(WIWR_OCC ~ canopy_relief_ratio+ elev_0pnt15_to_2pnt00_return_proportion+ elev_2pnt00_to_4pnt00_return_proportion+ elev_kurtosis+ elev_maximum +  elev_p50 + elev_stddev+ percentage_first_returns_above_2pnt00+ percentage_first_returns_above_mean+ total_all_returns +  (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = WIWR_OFF,  na.action = na.exclude ) 

l_vif<-as.data.frame(vif(plm5))

plm6<-lme4::glmer(WIWR_OCC ~ elev_0pnt15_to_2pnt00_return_proportion+ elev_2pnt00_to_4pnt00_return_proportion+ elev_kurtosis+ elev_maximum +  elev_p50 + elev_stddev+ percentage_first_returns_above_2pnt00+ percentage_first_returns_above_mean+ total_all_returns +  (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = WIWR_OFF,  na.action = na.exclude )
...
# [3] "elev_kurtosis"                           "elev_maximum"                           
# [5] "elev_p50"                                "elev_stddev"                            
# [7] "percentage_first_returns_above_2pnt00"   "percentage_first_returns_above_mean"    
# [9] "total_all_returns"     
...
```{r eval=FALSE}
in_1<-lme4::glmer(WIWR_OCC ~ elev_maximum  * elev_stddev    +  (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = WIWR_OFF,  na.action=na.fail) 
d_in_1<-MuMIn::dredge(in_1, extra = c(r2=function(x) round(r.squaredGLMM(x)[1,c(1,2)],3)))


in_2<-lme4::glmer(WIWR_OCC ~ elev_maximum *  elev_0pnt15_to_2pnt00_return_proportion +  (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = WIWR_OFF,  na.action=na.fail) 
d_in_2<-MuMIn::dredge(in_2, extra = c(r2=function(x) round(r.squaredGLMM(x)[1,c(1,2)],3)))


in_3<-lme4::glmer(WIWR_OCC ~ elev_maximum *  canopy_relief_ratio +  (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = WIWR_OFF,  na.action=na.fail) 
d_in_3<-MuMIn::dredge(in_3, extra = c(r2=function(x) round(r.squaredGLMM(x)[1,c(1,2)],3)))


in_4<-lme4::glmer(WIWR_OCC ~ elev_maximum  * elev_p50    +  (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = WIWR_OFF,  na.action=na.fail) 
d_in_4<-MuMIn::dredge(in_4, extra = c(r2=function(x) round(r.squaredGLMM(x)[1,c(1,2)],3)))

in_5<-lme4::glmer(WIWR_OCC ~ elev_stddev  :  elev_0pnt15_to_2pnt00_return_proportion    +  (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = WIWR_OFF,  na.action=na.fail) 
d_in_5<-MuMIn::dredge(in_5, extra = c(r2=function(x) round(r.squaredGLMM(x)[1,c(1,2)],3)))

#found elev_maximum  * elev_stddev 
#elev_maximum *  elev_0pnt15_to_2pnt00_return_proportion 
# elev_maximum *  canopy_relief_ratio
...
``` {r eval=FALSE}
# WIWR_can<-lme4::glmer(WIWR ~  canopy_relief_ratio +  elev_maximum + elev_stddev + total_all_returns +  (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = WIWR_OFF,  na.action=na.fail) 

WIWR_can<-lme4::glmer(WIWR_OCC ~ elev_0pnt15_to_2pnt00_return_proportion +elev_2pnt00_to_4pnt00_return_proportion+elev_kurtosis+elev_maximum * elev_stddev +elev_p50  +percentage_first_returns_above_2pnt00 +percentage_first_returns_above_mean  +total_all_returns +  (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = WIWR_OFF,  na.action = na.fail)
...
```{r eval=FALSE}
# m1<-lme4::glmer(WIWR_OCC ~  canopy_relief_ratio +  elev_maximum  + total_all_returns +  (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = WIWR_OFF,  na.action=na.fail) 

m1<-lme4::glmer(WIWR_OCC ~  elev_0pnt15_to_2pnt00_return_proportion +elev_2pnt00_to_4pnt00_return_proportion+elev_p50+percentage_first_returns_above_mean + (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = WIWR_OFF,  na.action = na.fail)
...
# m1: WIWR_OCC ~ elev_0pnt15_to_2pnt00_return_proportion + elev_2pnt00_to_4pnt00_return_proportion + elev_p50 + percentage_first_returns_above_mean + (1 | SS)
#         npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)    
# WIWR_m1    2 471.88 479.56 -233.94   467.88                         
...
#models
md_0<-lme4::glmer(WIWR_OCC ~  elev_0pnt15_to_2pnt00_return_proportion +elev_2pnt00_to_4pnt00_return_proportion+elev_p50+percentage_first_returns_above_mean + (1|SS), data=dd, family = binomial, offset = WIWR_OFF,  na.action = na.fail, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e9)))
...
md_1<-lme4::glmer(WIWR_OCC ~  elev_0pnt15_to_2pnt00_return_proportion +elev_2pnt00_to_4pnt00_return_proportion+elev_p50+percentage_first_returns_above_mean + (1|SS), data=dd, family = binomial, offset = WIWR_OFF,  na.action = na.exclude, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e9)) )
...
md_2<-lme4::glmer(WIWR_OCC ~  elev_0pnt15_to_2pnt00_return_proportion +elev_2pnt00_to_4pnt00_return_proportion+elev_p50+percentage_first_returns_above_mean + (1|SS), data=dd, family = binomial, offset = WIWR_OFF,  na.action = na.exclude, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e9)) )
...
md_3<-lme4::glmer(WIWR_OCC ~  elev_0pnt15_to_2pnt00_return_proportion +elev_2pnt00_to_4pnt00_return_proportion+elev_p50+percentage_first_returns_above_mean + (1|SS), data=dd, family = binomial, offset = WIWR_OFF,  na.action = na.exclude, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e9)) )
...
md_4<-lme4::glmer(WIWR_OCC ~  elev_0pnt15_to_2pnt00_return_proportion +elev_2pnt00_to_4pnt00_return_proportion+elev_p50+percentage_first_returns_above_mean + (1|SS), data=dd, family = binomial, offset = WIWR_OFF,  na.action = na.exclude, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e9)) )
...
md_5<-lme4::glmer(WIWR_OCC ~  elev_0pnt15_to_2pnt00_return_proportion +elev_2pnt00_to_4pnt00_return_proportion+elev_p50+percentage_first_returns_above_mean + (1|SS), data=dd, family = binomial, offset = WIWR_OFF,  na.action = na.exclude, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e9)) )
...
md_6<-lme4::glmer(WIWR_OCC ~  elev_0pnt15_to_2pnt00_return_proportion +elev_2pnt00_to_4pnt00_return_proportion+elev_p50+percentage_first_returns_above_mean + (1|SS), data=dd, family = binomial, offset = WIWR_OFF,  na.action = na.exclude, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e9)) )
...
md_7<-lme4::glmer(WIWR_OCC ~  elev_0pnt15_to_2pnt00_return_proportion +elev_2pnt00_to_4pnt00_return_proportion+elev_p50+percentage_first_returns_above_mean + (1|SS), data=dd, family = binomial, offset = WIWR_OFF,  na.action = na.exclude, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e9)) )
...
md_8<-lme4::glmer(WIWR_OCC ~  elev_0pnt15_to_2pnt00_return_proportion +elev_2pnt00_to_4pnt00_return_proportion+elev_p50+percentage_first_returns_above_mean + (1|SS), data=dd, family = binomial, offset = WIWR_OFF,  na.action = na.exclude, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e9)) )
...
md_9<-lme4::glmer(WIWR_OCC ~  elev_0pnt15_to_2pnt00_return_proportion +elev_2pnt00_to_4pnt00_return_proportion+elev_p50+percentage_first_returns_above_mean + (1|SS), data=dd, family = binomial, offset = WIWR_OFF,  na.action = na.exclude, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e9)) )
...
md_10<-lme4::glmer(WIWR_OCC ~  elev_0pnt15_to_2pnt00_return_proportion +elev_2pnt00_to_4pnt00_return_proportion+elev_p50+percentage_first_returns_above_mean + (1|SS), data=dd, family = binomial, offset = WIWR_OFF,  na.action = na.exclude, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e9)) )
...
md_11<-lme4::glmer(WIWR_OCC ~  elev_0pnt15_to_2pnt00_return_proportion +elev_2pnt00_to_4pnt00_return_proportion+elev_p50+percentage_first_returns_above_mean + (1|SS), data=dd, family = binomial, offset = WIWR_OFF,  na.action = na.exclude, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e9)) )
...
md_12<-lme4::glmer(WIWR_OCC ~  elev_0pnt15_to_2pnt00_return_proportion +elev_2pnt00_to_4pnt00_return_proportion+elev_p50+percentage_first_returns_above_mean + (1|SS), data=dd, family = binomial, offset = WIWR_OFF,  na.action = na.exclude, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e9)) )
...
md_13<-lme4::glmer(WIWR_OCC ~  elev_0pnt15_to_2pnt00_return_proportion +elev_2pnt00_to_4pnt00_return_proportion+elev_p50+percentage_first_returns_above_mean + (1|SS), data=dd, family = binomial, offset = WIWR_OFF,  na.action = na.exclude, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e9)) )
...
md_14<-lme4::glmer(WIWR_OCC ~  elev_0pnt15_to_2pnt00_return_proportion +elev_2pnt00_to_4pnt00_return_proportion+elev_p50+percentage_first_returns_above_mean + (1|SS), data=dd, family = binomial, offset = WIWR_OFF,  na.action = na.exclude, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e9)) )
...
md_15<-lme4::glmer(WIWR_OCC ~  elev_0pnt15_to_2pnt00_return_proportion +elev_2pnt00_to_4pnt00_return_proportion+elev_p50+percentage_first_returns_above_mean + (1|SS), data=dd, family = binomial, offset = WIWR_OFF,  na.action = na.exclude, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e9)) )
...
WIWR_fa_co_df<-as.data.frame(na.omit(values(fa_co)))%>%
  dplyr::rename(forest_age=lidar_forestAge_raster)
          
save(WIWR_fa_co_df, file="2_pipeline/store/WIWR_fa_co_df.rData")

load("2_pipeline/store/WIWR_fa_co_df.rData")


#######################################################################################
############### Plot Effect ######################################################
#######################################################################################
lm_fa_00to01 <- lm(predCor_0_01 ~ forest_age, data=WIWR_fa_co_df)
t<-ggpredict(lm_fa_00to01, terms="forest_age [all]", data=WIWR_fa_co_df)
...
dev.off()
#######################################################################################
lm_fa_00to02 <- lm(predCor_0_02 ~ forest_age, data=WIWR_fa_co_df)
t<-ggpredict(lm_fa_00to02, terms="forest_age [all]", data=WIWR_fa_co_df)
...
dev.off()
#######################################################################################
lm_fa_00to03 <- lm(predCor_0_03 ~ forest_age, data=WIWR_fa_co_df)
t<-ggpredict(lm_fa_00to03, terms="forest_age [all]", data=WIWR_fa_co_df)
...
dev.off()
#######################################################################################
lm_fa_00to04 <- lm(predCor_0_04 ~ forest_age, data=WIWR_fa_co_df)
t<-ggpredict(lm_fa_00to04, terms="forest_age [all]", data=WIWR_fa_co_df)
...
dev.off()
#######################################################################################
lm_fa_00to05 <- lm(predCor_0_05 ~ forest_age, data=WIWR_fa_co_df)
t<-ggpredict(lm_fa_00to05, terms="forest_age [all]", data=WIWR_fa_co_df)
...
dev.off()
#######################################################################################
lm_fa_00to06 <- lm(predCor_0_06 ~ forest_age, data=WIWR_fa_co_df)
t<-ggpredict(lm_fa_00to06, terms="forest_age [all]", data=WIWR_fa_co_df)
...
dev.off()
#######################################################################################
lm_fa_00to07 <- lm(predCor_0_07 ~ forest_age, data=WIWR_fa_co_df)
t<-ggpredict(lm_fa_00to07, terms="forest_age [all]", data=WIWR_fa_co_df)
...
dev.off()
#######################################################################################
lm_fa_00to08 <- lm(predCor_0_08 ~ forest_age, data=WIWR_fa_co_df)
t<-ggpredict(lm_fa_00to08, terms="forest_age [all]", data=WIWR_fa_co_df)
...
dev.off()
#######################################################################################
lm_fa_00to09 <- lm(predCor_0_09 ~ forest_age, data=WIWR_fa_co_df)
t<-ggpredict(lm_fa_00to09, terms="forest_age [all]", data=WIWR_fa_co_df)
...
dev.off()
#######################################################################################
lm_fa_00to10 <- lm(predCor_0_10 ~ forest_age, data=WIWR_fa_co_df)
t<-ggpredict(lm_fa_00to10, terms="forest_age [all]", data=WIWR_fa_co_df)
...
dev.off()
#######################################################################################
lm_fa_00to11 <- lm(predCor_0_11 ~ forest_age, data=WIWR_fa_co_df)
t<-ggpredict(lm_fa_00to11, terms="forest_age [all]", data=WIWR_fa_co_df)
...
dev.off()
#######################################################################################
lm_fa_00to12 <- lm(predCor_0_12 ~ forest_age, data=WIWR_fa_co_df)
t<-ggpredict(lm_fa_00to12, terms="forest_age [all]", data=WIWR_fa_co_df)
...
dev.off()
#######################################################################################
lm_fa_00to13 <- lm(predCor_0_13 ~ forest_age, data=WIWR_fa_co_df)
t<-ggpredict(lm_fa_00to13, terms="forest_age [all]", data=WIWR_fa_co_df)
...
dev.off()
#######################################################################################
lm_fa_00to14 <- lm(predCor_0_14 ~ forest_age, data=WIWR_fa_co_df)
t<-ggpredict(lm_fa_00to14, terms="forest_age [all]", data=WIWR_fa_co_df)
...
dev.off()
#######################################################################################
lm_fa_00to15 <- lm(predCor_0_15 ~ forest_age, data=WIWR_fa_co_df)
t<-ggpredict(lm_fa_00to15, terms="forest_age [all]", data=WIWR_fa_co_df)
...
knitr::kable(head(WIWR_predCor_to_forestAge_df)) %>%
  kable_styling(font_size = 10, bootstrap_options= c("striped", "hover", "condensed" ), full_width= F, position="center") %>%
  column_spec(1, width= "20em")%>%
  scroll_box(width = "100%", height = "400px")
```
