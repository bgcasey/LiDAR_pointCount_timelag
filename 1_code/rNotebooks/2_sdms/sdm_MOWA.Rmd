...
plm2<-lme4::glmer(MOWA ~ rast_forest_age + canopy_relief_ratio+ elev_cv+ elev_kurtosis+ elev_maximum + elev_mean+ elev_p05 + elev_p10 + elev_p20 + elev_p25 + elev_p30 + elev_p50 + elev_p60 + elev_p70 + elev_p75 + elev_p80 + elev_p95 + elev_p99 + elev_stddev+ percentage_first_returns_above_mean+ total_all_returns +  (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = MOWA_OFF,  na.action = na.exclude )  
l_vif<-as.data.frame(vif(plm2))


plm3<-lme4::glmer(MOWA ~ rast_forest_age + canopy_relief_ratio+ elev_cv+ elev_kurtosis+ elev_maximum + elev_mean+ elev_p50 + + elev_stddev+ percentage_first_returns_above_mean+ total_all_returns +  (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = MOWA_OFF,  na.action = na.exclude )

l_vif<-as.data.frame(vif(plm3))


plm4<-lme4::glmer(MOWA ~ canopy_relief_ratio+ elev_maximum +  elev_p50 + elev_stddev+ percentage_first_returns_above_mean+ total_all_returns +  (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = MOWA_OFF,  na.action = na.exclude )

l_vif<-as.data.frame(vif(plm4))


plm5<-plm4<-lme4::glmer(MOWA ~ elev_maximum + elev_stddev+ percentage_first_returns_above_mean+ total_all_returns +  (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = MOWA_OFF,  na.action = na.exclude )
...
```{r eval=FALSE}
m1<-lme4::glmer(MOWA ~  canopy_relief_ratio +  elev_maximum  + total_all_returns +  (1|SS), data=d, subset=SS_lidar_timelag==0, family = binomial, offset = MOWA_OFF,  na.action=na.fail) 
...
md_0<-glm(MOWA ~  canopy_relief_ratio +  elev_maximum + elev_stddev + total_all_returns, data=train, family = binomial, offset = MOWA_OFF,  na.action=na.fail)
...
```{r eval=FALSE}
d<-sdm_d%>%filter(SS_lidar_timelag==0)
...
auc(test$MOWA, predicted)

AUCFun <- function(fit) {
  library(pROC)
  pred<-predict(fit, test, type="response",allow.new.levels=TRUE)
  AUC<-as.numeric(auc(test$MOWA, pred))
  }

#test
m<-AUCFun(md_1)

test<-bootMer(md_0, nsim = 100, seed = 101, FUN=AUCFun,
        type = "semiparametric",
...
predicted <- predict(m_0, test, type="response")
...
###### Run tup model at each time lag
...
md_0<-glm(MOWA ~  canopy_relief_ratio +  elev_maximum  + total_all_returns, data=train, family = binomial, offset = MOWA_OFF,  na.action=na.fail) 
predicted <- predict(md_0, test, type="response")
auc_d_0<-auc(test$MOWA, predicted)
...
md_1<-glm(MOWA ~   canopy_relief_ratio +  elev_maximum + elev_stddev + total_all_returns, data=train, family = binomial, offset = MOWA_OFF,  na.action=na.fail)
...
md_2<-glm (MOWA ~   canopy_relief_ratio +  elev_maximum + elev_stddev + total_all_returns, data=train, family = binomial, offset = MOWA_OFF,  na.action=na.fail)
...
md_3<-glm (MOWA ~   canopy_relief_ratio +  elev_maximum + elev_stddev + total_all_returns, data=train, family = binomial, offset = MOWA_OFF,  na.action=na.fail)
...
md_4<-glm (MOWA ~   canopy_relief_ratio +  elev_maximum + elev_stddev + total_all_returns, data=train, family = binomial, offset = MOWA_OFF,  na.action=na.fail)
...
md_5<-glm (MOWA ~   canopy_relief_ratio +  elev_maximum + elev_stddev + total_all_returns, data=train, family = binomial, offset = MOWA_OFF,  na.action=na.fail)
...
md_6<-glm (MOWA ~   canopy_relief_ratio +  elev_maximum + elev_stddev + total_all_returns, data=train, family = binomial, offset = MOWA_OFF,  na.action=na.fail)
...
md_7<-glm (MOWA ~   canopy_relief_ratio +  elev_maximum + elev_stddev + total_all_returns, data=train, family = binomial, offset = MOWA_OFF,  na.action=na.fail)
...
md_8<-glm (MOWA ~   canopy_relief_ratio +  elev_maximum + elev_stddev + total_all_returns, data=train, family = binomial, offset = MOWA_OFF,  na.action=na.fail)
...
md_9<-glm (MOWA ~   canopy_relief_ratio +  elev_maximum + elev_stddev + total_all_returns, data=train, family = binomial, offset = MOWA_OFF,  na.action=na.fail)
...
md_10<-glm (MOWA ~   canopy_relief_ratio +  elev_maximum + elev_stddev + total_all_returns, data=train, family = binomial, offset = MOWA_OFF,  na.action=na.fail)
...
md_11<-glm (MOWA ~   canopy_relief_ratio +  elev_maximum + elev_stddev + total_all_returns, data=train, family = binomial, offset = MOWA_OFF,  na.action=na.fail)
...
md_12<-glm (MOWA ~   canopy_relief_ratio +  elev_maximum + elev_stddev + total_all_returns, data=train, family = binomial, offset = MOWA_OFF,  na.action=na.fail)
...
md_13<-glm (MOWA ~   canopy_relief_ratio +  elev_maximum + elev_stddev + total_all_returns, data=train, family = binomial, offset = MOWA_OFF,  na.action=na.fail)
...
md_14<-glm (MOWA ~   canopy_relief_ratio +  elev_maximum + elev_stddev + total_all_returns, data=train, family = binomial, offset = MOWA_OFF,  na.action=na.fail)
...
md_15<-glm (MOWA ~   canopy_relief_ratio +  elev_maximum + elev_stddev + total_all_returns, data=train, family = binomial, offset = MOWA_OFF,  na.action=na.fail)
...
####### Confidence intervals on AUCs

###### Compare spearman's correlation {-}
